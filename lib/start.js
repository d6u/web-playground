...
module.exports = (opts) => {
  ...
  File.watch(joinBase('playground.*'))
    ...
    .flatMapLatest(R.converge(
      ...
      (playground) => File.watch(joinBase(`html.{${Assets.getHtmlExts(playground).join(',')}}`))
        .flatMap(File.readFile)
        .flatMap(Assets.getHtmlRenderer(playground))
        .doOnNext(serveAssets.updateAsset('html')),
      (playground) => File.watch(joinBase(`js.{${Assets.getJsExts(playground).join(',')}}`))
        .flatMap(File.readFile)
        .flatMap(Assets.getJsRenderer(playground))
        .doOnNext(serveAssets.updateAsset('js')),
      (playground) => File.watch(joinBase(`css.{${Assets.getCssExts(playground).join(',')}}`))
        .flatMap(File.readFile)
