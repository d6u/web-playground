...
module.exports = function () {
  tryInOrder(PLAYGROUND_FILES, readFile)
    ...
    .subscribeOnNext(function (playground) {
      createAssetFile(playground, 'html', function (ext) {
        return ext === 'html' ? readFile(TMPL_HTML) : just('');
      });

      createAssetFile(playground, 'js', function (ext) {
        return just('');
      });

      createAssetFile(playground, 'css', function (ext) {
        return just('');
      });
    });
};
