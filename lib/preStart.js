...
var readFileObs = Observables.observablefyPromise(File.readFile);
...
module.exports = function preStart(start) {
  ...
  Observables
    .tryInOrder(CONST.PLAYGROUND_FILES, readFileObs)
    .catch(function (err) {
      ...
      return readFileObs(CONST.PLAYGROUND_EXAMPLE)
    ...
    .flatMap(function (playground) {
      var htmlObs = Playground.createAssetFile(playground, 'html', function (ext) {
        hasCreatedFromTmpl = true;
        return ext === 'html' ? readFileObs(CONST.HTML_EXAMPLE) : Observable.just('');
      });
